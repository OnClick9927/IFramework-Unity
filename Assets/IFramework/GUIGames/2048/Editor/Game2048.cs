 ...
 *History:        2018.11--
*********************************************************************************/
using IFramework.GUIDesign.LayoutDesign;
using System.Collections.Generic;
using System.Xml;
using UnityEditor;
using UnityEngine;

namespace IFramework.GUIGames
{
    [EditorWindowCache("Game_2048")]
	public class Game2048:EditorWindow
	{
        ...
        private void OnEnable()
        {
            ...
            string path= FrameworkConfig.FrameworkPath.CombinePath("GUIGames/2048/Resources/Game_2048.xml");
            doc.Load(path);
        ...
        {
            guiCanvas.Find<TextLabel>("Horizontal/score").text = Score.ToString();
            for (int i = 0; i < size; i++)
            {
                for (int j = 0; j < size; j++)
                {
                    int val = arr[i, j];
                    if (val != 0)
                        guiCanvas.Find<TextLabel>(string.Format("map/{0}/{1}", i + 1, j + 1)).text = val.ToString();
                    else
                        guiCanvas.Find<TextLabel>(string.Format("map/{0}/{1}", i + 1, j + 1)).text = "";
                }
            }
        }
        private void OnGUI()
