...
namespace IFramework
{
    ...
    public class EditorWindowCacheAttribute : Attribute
    {
        ...
        public EditorWindowCacheAttribute(string searchName, string menuPath)
        {
            this.searchName = searchName;
    ...
    class EditorWindowUtil
    {
        public class EditorWindowInfo
        {
            ...
            public Type type;
            public EditorWindowInfo(Type type, string menuPath = null, string SearchName = "")
            {
                this.menuPath = menuPath;
                this.type = type;
            ...
            foreach (var item in typeof(EditorWindow).GetSubTypesInAssemblys())
            {
                ...
                for (int i = 0; i < attrs.Length; i++)
                {
                    if (attrs[i] is EditorWindowCacheAttribute)
                    {
                        ...
                        Windows.Add(new EditorWindowInfo(item, attr.menuPath, attr.searchName));
                    }
                }
            }
