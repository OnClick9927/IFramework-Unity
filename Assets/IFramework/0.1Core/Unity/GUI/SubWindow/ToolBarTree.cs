...
{
    public class ToolBarTree: ILayoutGUIDrawer
    {
        ...
        private class ToolBarSearchField : ToolbarNode
        {
            ...
            public ToolBarSearchField(GUIContent content, Action<string> onValueChange, string value, int width = 100) : base(content, width)
            {
                ...
                s = new SearchFieldDrawer() { value=value,onValueChange=(str)=> {
        ...
        public void OnGUI(Rect position)
        {
            this.BeginArea(position)
                    .BeginHorizontal(Styles.ToolBar, GUILayout.Width(position.width))
                        .Pan(() => {Nodes.ForEach((n) => { n.OnGUI(); });})
                    .EndHorizontal()
                .EndArea();
        }
